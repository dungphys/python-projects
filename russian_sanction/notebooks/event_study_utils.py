...
EST_START, EST_END = -120, -21     # estimation window (trading days rel. to cluster anchor)
EVT_PRE, EVT_POST = -1, 10         # event window (trading days rel. to cluster anchor)
CLUSTER_GAP = 5                    # trading days: gap <= this => same cluster
...
def cluster_events(sanctions_csv, mkt):
    ...
    clusters = ev.groupby('cluster_id').agg(
        t0=('t', 'min'), date0=('date', 'min'), date1=('date', 'max'),
...
def build_window_metadata(clusters):
    ...
    max_t = clusters['t0'].max() + 10_000  # placeholder, replaced by caller with mkt max
    rows = []
    for _, row in clusters.iterrows():
        t0 = row['t0']
        est_lo, est_hi = t0 + EST_START, t0 + EST_END
        evt_lo, evt_hi = t0 + EVT_PRE, t0 + EVT_POST

        other_t0 = clusters.loc[clusters['cluster_id'] != row['cluster_id'], 't0']
        contaminated = bool(other_t0.between(est_lo, evt_hi).any())

        est_days = np.arange(est_lo, est_hi + 1)
        other_evt = [(r['t0'] + EVT_PRE, r['t0'] + EVT_POST)
        ...
        rows.append(dict(
            cluster_id=row['cluster_id'], t0=t0, date0=row['date0'], date1=row['date1'],
...
def compute_car_for_cluster(mkt, mrow, asset_key, min_est_obs=20):
    lvl_col, ret_col = ASSET_COLS[asset_key]
    ...
    p_start = _last_avail(mkt, lvl_col, mrow['evt_lo'] - 1)
    exact_end = mkt.loc[mkt['t'] == mrow['evt_hi'], lvl_col]
    if len(exact_end) and not exact_end.isna().all():
    ...
    raw_cum = p_end - p_start
    car = raw_cum - mu * L
    scar = car / (sigma * np.sqrt(L))
...
def bmp_test(scar_series):
    ...
    mean_scar = x.mean()
    sd = x.std(ddof=1)
    bmp_t = np.sqrt(n) * mean_scar / sd if sd > 0 else np.nan
